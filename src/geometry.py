...
def triangulate_points(P1, P2, points1, points2):
    ...
    """
    points1 = np.asarray(points1, dtype=np.float32).T
    points2 = np.asarray(points2, dtype=np.float32).T
...
def filter_by_cheirality(points_3d, mask):
    """
    Filter points using cheirality check (points in front of both cameras).
    """
    if mask is None:
        return points_3d
